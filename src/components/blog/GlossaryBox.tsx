      ...
      <div className="px-4 py-4 border-b border-gray-100 dark:border-white/5 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[13px] text-gray-500 dark:text-gray-400 mt-0.5 font-medium">
            어려운 손해사정 전문 용어를 쉽게 설명해 드립니다 (클릭 시 상세 해설)
          </p>
        </div>
      </div>
        ...
        {items.map((item, i) => (
          <div key={i}>
            ...
            >
              ...
              <span className={`flex-1 text-[15px] transition-colors break-keep ${openIdx === i ? 'font-extrabold text-[#34A853] dark:text-[#81c995]' : 'font-semibold text-gray-800 dark:text-[#e8eaed]'}`}>
                {item.term}
              </span>
              <svg
                className={`w-4 h-4 shrink-0 transition-transform duration-300 ${openIdx === i ? 'rotate-180 text-[#34A853]' : 'text-gray-400'}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                strokeWidth="2.5"
            ...
            >
              <div className="px-4 pb-4 pt-1.5">
                <div className="flex gap-3.5 p-4 rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-white/5">
                  <span className="w-8 h-8 rounded-full bg-[#34A853] text-white text-[13px] font-bold flex items-center justify-center shrink-0">
                    A
                  </span>
                  <div
                    className="text-[14.5px] text-gray-700 dark:text-gray-300 leading-[1.8] flex-1 break-keep"
                    dangerouslySetInnerHTML={{
                      __html: item.definition.replace(
                        /\*\*(.+?)\*\*/g,
